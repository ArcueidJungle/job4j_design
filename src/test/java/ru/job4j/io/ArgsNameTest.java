...
class ArgsNameTest {
    ...
    @Test
    void whenHyphenPrefixAndNoKeyThenExceptionThrown() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-=512", "-=?msg=Exit="}))
                .isInstanceOf((IllegalArgumentException.class))
                .hasMessageMatching("^.+")
    ...
    void whenStringDoesNotContainValueThenExceptionThrown3() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-smx=", "-xlx=message"}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument '-smx=' does not contain a value");
    }
